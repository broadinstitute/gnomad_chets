...
| compute_vp_counts | `--create-dense-filtered-mt` | gnomAD VDS, pair list | `filtered.dense.{postfix}.mt` |
| compute_vp_counts | `--encode-genotypes` | dense MT | `genotype_count_intermediates.{postfix}/...` |
| compute_vp_counts | `--compute-counts-per-sample` | dense MT, pair list, filter HT | `variant_pairs.genotype_counts.{postfix}.ht` |
