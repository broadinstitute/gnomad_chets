...
def main(args):
    ...
    hc = hail.HailContext(log='/concordance.log')

    raw_hardcalls_split_path = full_genome_hardcalls_split_vds if (args.genomes) else full_exome_hardcalls_split_vds
    if args.compute_syndip_concordance:
        ...
        compute_concordance(left_vds=hc.read(raw_hardcalls_split_path),
                            right_vds= hc.read(syndip_path).rename_samples(sample_map),
                            out_prefix = args.output + ".syndip",
                            high_conf_regions = syndip_high_conf_regions_path,
    ...
    if args.compute_na12878_concordance:
        high_conf_regions = NA12878_high_conf_regions_path if (args.genomes) else NA12878_high_conf_exome_regions_path
        sample_map = {'INTEGRATION': 'G94982_NA12878'} if (args.genomes) else {'INTEGRATION': 'C1975::NA12878'}
        compute_concordance(left_vds= hc.read(raw_hardcalls_split_path),
                            right_vds= hc.read(NA12878_path).rename_samples(sample_map),
                            samples = sample_map.values,
    ...
    if args.compute_omes_concordance:
        exomes = hc.read(full_exome_hardcalls_split_vds).filter_samples_list(read_list_data(exomes_qc_pass_samples))
        exomes = rename_samples(exomes, exomes_to_combined_IDs)

        genomes = hc.read(full_genome_hardcalls_split_vds).filter_samples_list(read_list_data(genomes_qc_pass_samples))
        genomes = rename_samples(genomes, genomes_to_combined_IDs)

        compute_concordance(left_vds= genomes,
                            right_vds= exomes,
                            high_conf_regions = exomes_high_conf_regions_path,
    ...
    if args.write_omes_interval_concordance:
        ...
        intervals_kt = hail.KeyTable.import_interval_list(exomes_high_conf_regions_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--compute_syndip_concordance', help='Computes syndip concordance', action='store_true')
    parser.add_argument('--compute_na12878_concordance', help='Computes NA12878 concordance', action='store_true')
    parser.add_argument('--compute_omes_concordance', help='Computes exomes/genomes concordance', action='store_true')
    parser.add_argument('--write_syndip_concordance', help='Write syndip concordance', action='store_true')
    parser.add_argument('--write_na12878_concordance', help='Write NA12878 concordance', action='store_true')
    parser.add_argument('--write_omes_concordance', help='Write exomes/genomes concordance', action='store_true')
    parser.add_argument('--write_omes_interval_concordance', help='Write exomes/genomes concordance by interval', action='store_true')
    parser.add_argument('--rf_ann_files',
                        help='Annotation files to annotate RF results in pipe-delimited format: name|location|rf_root', nargs='+')
