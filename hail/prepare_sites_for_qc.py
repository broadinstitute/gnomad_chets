...
root = 'gs://gnomad/sampleqc'

cpg_vds_path = 'gs://gnomad-public/cpg.vds'

full_vds_path = 'gs://gnomad-exomes-raw/full/gnomad.exomes.all.vds'
common_sites_vds_path = 'gs://gnomad-exomes-raw/hardcalls/gnomad.exomes.common_SNPs.vds'
...
if get_exome_data:
    ...
    exome_vds = (hc.read(full_vds_path)
                 .annotate_variants_expr(['va.calldata.all_samples_raw = gs.callStats(g => v)', 'va.callrate = gs.fraction(g => g.isCalled)'])
                 .annotate_variants_vds(cpg_vds, code='va.cpg = !isMissing(vds)')
                 .filter_variants_expr('v.isBiallelic && !va.cpg && v.altAllele.isSNP && v.isAutosomal && va.calldata.all_samples_raw.AF[1] > 0.001 && va.callrate > 0.99')
                 .hardcalls()
                 .write(common_sites_vds_path))

if get_genome_data:
    cpg_vds = hc.read(cpg_vds_path)
    genome_vds = (hc.read(full_genome_vds_path)
                  .filter_samples_list(genome_sample_list, keep=False)
                  .annotate_variants_expr(['va.calldata.all_samples_raw = gs.callStats(g => v)', 'va.callrate = gs.fraction(g => g.isCalled)'])
                  .annotate_variants_vds(cpg_vds, code='va.cpg = !isMissing(vds)')
                  .filter_variants_expr('v.isBiallelic && !va.cpg && v.altAllele.isSNP && v.isAutosomal && va.calldata.all_samples_raw.AF[1] > 0.001 && va.callrate > 0.99')
                  .hardcalls()
                  .write(common_sites_genome_vds_path))

if join_exomes_genome:
    genome_sample_rename = '/tmp/genome_ids.txt'
    exome_sample_rename = '/tmp/exome_ids.txt'

    exome_vds = hc.read(common_sites_vds_path)
    ...
    (exome_vds.rename_samples('file://' + exome_sample_rename).annotate_samples_expr('sa = {}')
     .join(genome_vds.rename_samples('file://' + genome_sample_rename).annotate_samples_expr('sa = {}'))
