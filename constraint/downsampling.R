...
proportion_observed_graphs = function() {
...
constraint = function() {
  ...
  subset(all_transcripts, annotation == 'synonymous_variant' & (property %in% c('variant_count', 'expected_variant_count_adj'))) %>% 
    ggplot + aes(x=n, y=num, col=property) + geom_line(size=1) + 
    xlab('Number of individuals') + ylab('Variant Count') + theme_classic()
  ...
  all_transcripts %>%
    ...
    ggplot + aes(x=n, y=num, col=annotation, linetype=property) + geom_line(size=1) + 
    xlab('Number of individuals') + ylab('Variant Count') + theme_classic()
  ...
  final_constraint = obs_exp %>% filter(n == 123136)
